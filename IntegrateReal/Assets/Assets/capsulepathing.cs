...
public class capsulepathing : MonoBehaviour
{
    ...
    public Vector3 offs;
    private Vector3 targetPosition = new Vector3(-1.0f, 0.0f, 1.54f);
    ...
    public void Update()
    {
        ...
        // Go to next point
        //POINT 1
        if ((AbsVal(transform.position.x, targetPosition.x) < 0.5f) && (x < 2))
        {
        ...
        //POINT 2
        else if ((AbsVal(transform.position.x, targetPosition.x) < 0.5f) && (y < 2))
        {
        ...
        else if ((AbsVal(transform.position.x, targetPosition.x) < 0.5f) && (z < 2))
        {
            ...
            newTarg = new Vector3(-2.0f, 0.0f, -1.287f);
            targetPosition = newTarg;
            currentWaypoint = 0;
            z++;
            Update();
